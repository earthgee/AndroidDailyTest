...
char *jstringToChars2(JNIEnv *env, jstring jstr) {
    if (jstr == nullptr) {
        return nullptr;
    }
...
void printJavaStack2() {
    ...
    char* stack = jstringToChars2(jniEnv, java_stack);
    ALOG("stack:%s", stack);
    free(stack);
...
JNI_METHOD_DECL(void, enableSocketHookNative)
...
static bool InitJniEnv(JavaVM *vm) {
    ...
    kJavaClass = reinterpret_cast<jclass>(env->NewGlobalRef(env->FindClass("com/earthgee/systrace/ATrace")));
    if (kJavaClass == NULL)  {
